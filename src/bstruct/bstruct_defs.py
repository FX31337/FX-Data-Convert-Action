...
class FxtHeader(BStruct):
    ...
    _fields = [
        ...
        (
            'headerVersion',
            'I',
        ),  # Version      uint32      0   4 Header version (default: 405).
        (
            'copyright',
            '64s',
            pretty_print_string,
        ...
        (
            'digits',
            'I',
        ),  # Digits       uint32    256   4 Symbol digits = MarketInfo(MODE_DIGITS)
        (
            'padding2',
            '4s',
            pretty_print_ignore,
        ...
        (
            'accountLeverage',
            'i',
        ),  # AccountLeverage           uint32   356    4 Account leverage = AccountLeverage(); (default: 100)
        (
            'freeMarginMode',
            'i',
...
class FxtTick(BStruct):
    ...
    _fields = [
        ...
        (
            ...
            'i',
            pretty_print_time,
        ),  # TickTimestamp uint32  48  4 Tick data timestamp in seconds (the current time within a bar).
        (
            'launchExpert',
            'i',
            pretty_print_time,
...
class HstBar(BStruct):
    ...
    _fields = [
        (
            ...
            'II',
            pretty_print_time,
        ),  # CTM           uint64  0   8 Current time in seconds aligned with timeframe (MQL4 datetime).
        ('open', 'd'),  # Open          float64 8   8
        ('high', 'd'),  # High          float64 16  8
        ('low', 'd'),  # Low           float64 24  8
        ('close', 'd'),  # Close         float64 32  8
        ...
        (
            'spread',
            'I',
        ),  # Spread        uint32  48  8 Spread in points, 0 for current online spread (variable).
...
class ExpertsIni(BStruct):
    ...
    _fields = [
        (
            'headerVersion',
            'I',
        ),  # Version                       uint32               0    4 Header version (default: 400).
        (
            'unknown_0',
            '44s',
        ),  # Unknown data                  [44]byte             4   47 Unknown data.
        (
            'webRequestUrlEnabled',
            'B',
        ),  # Enable URL white-listing      [1]unsigned char    48   48 Boolean: 0/1.
        (
            'unknown_1',
            '243s',
        ),  # Unknown data                  [245]byte           49  293 Unknown data.
        (
            'webRequestUrl',
            '4096s',
            pretty_print_wstring,
        ),  # Web Request URL               [2080]byte         294 4387 Account server name (szchar).
    ]
    _truncate = True
    _size = get_fields_size(_fields)
    assert _size == 4388
